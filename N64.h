...
N64::N64(){
  _joystick = new Joystick_(JOYSTICK_DEFAULT_REPORT_ID,JOYSTICK_TYPE_N64,
...
void N64::set_Button(word currentState, word lastState, byte index, word mask){
  if (isStateChanged(currentState, lastState, mask) )
    _joystick->setButton(index, (currentState & mask)>0);
}
void N64::set_Directional(word currentState, word lastState){
  //check up down
...
bool N64::isStateChanged(word currentState, word lastState, word mask){
